...
const vertexShader = `
  ...
  void main() {
    ...
    float noise3 = snoise(position * uFrequency * 0.5 + uTime * 0.15) * 1.5;
...
const fragmentShader = `
  ...
  void main() {
    ...
    float alpha = 0.15 + fresnel * 0.4 + vDisplacement * 0.1;
...
export function LiquidBlob({ position = [-4, 0, -3], scale = 2.5, speed = 1 }: Props) {
  ...
  useFrame((state) => {
    ...
    if (meshRef.current) {
      ...
      meshRef.current.position.y = position[1] + Math.sin(t * 0.3) * 0.5;
    }
  });
